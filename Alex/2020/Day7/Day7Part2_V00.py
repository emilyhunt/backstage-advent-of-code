...
with open("input_day7_test.txt") as rulesf:
	for line in rulesf:
		#I
		line = line[:-2]
...
#Find bags in shiny gold
contents = [["1 shiny gold"]]
#contents = findbags(contents[0][2:], rules)
m = 0
k = 0
while m < len(contents):
	m = len(contents)
	levelcontents = []
	for searchbag in contents[k]:
		#print(searchbag)
		newcontents = findbags(searchbag[2:], rules)
		if newcontents == []:
			break
