...
def list_elements(line):
    if line[0][0] == line[1][0]:#Vertical Line
        ...
        dir_x = int((line[1][0] - line[0][0]) / abs(line[1][0] - line[0][0]))
        x_coords = np.array(range(line[0][0],line[1][0]+1*dir_x,1*dir_x))
        dir_y = int((line[1][1] - line[0][1]) / abs(line[1][1] - line[0][1]))
        y_coords = np.array(range(line[0][1],line[1][1]+1*dir_y,1*dir_y))
